...
class ExtratoContaWidget extends Widget
{
    public $mes;
    public $ano;
    ...
    public function run()
    {
        ...
        if (!empty($this->data['cnt_plano_terceiro_id'])) {
            ...
            $html .= Html::tag('th', str_pad($this->terceiro['id'], 2, '0', STR_PAD_LEFT));
            $html .= Html::tag('th', $this->terceiro['nome'], ['colspan' => '8']);
            $html .= Html::endTag('tr');
        }
        $html .= Html::beginTag('tr');
    ...
    public static function getFilhosTable($data)
    {
        ...
        $query = Yii::$app->CntQuery->queryExtrato($data);
        $total_saldo = $total_saldo + Yii::$app->CntQuery->queryExtratoSaldoMesAnterior($data);
