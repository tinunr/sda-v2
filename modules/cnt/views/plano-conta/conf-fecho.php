...
$this->title = 'Configurar fecho de Conta';
?>
...
<?php $form = ActiveForm::begin(); ?>
    ...
    <div class="col-md-5">
      <?= $form->field($model, 'cnt_plano_conta_fecho')->widget(Select2::classname(), [
          'data' => ArrayHelper::map((new yii\db\Query)->from('cnt_plano_conta')->select(['id', new \yii\db\Expression("CONCAT(`id`, ' - ', `descricao`) as descricao")]) ->where(['cnt_plano_conta_tipo_id'=>2])->orderBy('id')->all(), 'id','descricao'),
          'theme' => Select2::THEME_BOOTSTRAP,
          'options' => ['placeholder' => 'Fechar em'],
          'pluginOptions' => [
                  'allowClear' =>true,
              ],
          ])->label(false)
...
<div class="table-responsive">
    <table class="table table-hover">
        ...
        <?php foreach($data as $value):?>
            <tr>
                <td>  <?= Html::a(Yii::$app->ImgButton->Img('delete'), ['conf-fecho-delete', 'id' => $value->id], [
            ...
            'data' => [
                ...
                <td><?=$value->cnt_plano_conta_fecho?></td>
                <td><?=$value->fecho->descricao?></td>
            </tr>
        <?php endforeach;?>
        </tbody>
    </table>
