...
$this->registerJsFile(Url::to('@web/js/nordData.js'),['position' => \yii\web\View::POS_END]);
...
<table class="container-items table table-hover">
  ...
  <tbody>
    <tr >
      ...
      <td><?= $form->field($model, 'honorario')->textInput(['id'=>'honorario','onchange'=>'updateIvaBYhonorario(this)'])->label(false) ?></td>
      <td  style="width: 50px"></td>
    </tr>
    <tr>
      <th scope="row"></th>
      <td> <strong>Iva sobre Honarario</strong></td>
      <td  style="width: 250px"><?= $form->field($model, 'iva_honorario')->textInput(['id'=>'iva_honorario'])->label(false) ?></td>
                 ...
                 <?php foreach ($modelsFaturaProvisoriaItem as $i => $item): ?>                    
                    ...
                    <?php
                        ...
                        <tr class="item" >
                          ...
                          <th style="width: 250px" > <?= $form->field($item, "[{$i}]valor")->textInput(['maxlength' => true])->label(false) ?> </th>
                          <th style="width: 50px" > <?= $form->field($item, "[{$i}]item_origem_id")->textInput(['readonly'=>true])->label(false) ?> </th>
                        </tr>
            <?php endforeach; ?>

            </table>
            <button id="add-item" onclick="addItem();" value=<?=$i?> type="button" class="add-item btn btn-success btn-xs"><i class="glyphicon glyphicon-plus"></i></button>
            <?=Html::hiddenInput('hidemCount', $i,['id'=>'hidemCount','class' => 'form-control']);?>
            <?php DynamicFormWidget::end(); ?>
    </div>


 <?= $form->field($model, 'valor')->textInput(['id' => 'total','readonly'=>true])->label('Total') ?>


    <div class="form-group">
        <?= Html::submitButton(Yii::$app->ImgButton->Img('save').' Salvar', ['class' => $model->isNewRecord ? 'btn btn-success' : 'btn btn-primary','title'=>'SALVAR']) ?>
    </div>
