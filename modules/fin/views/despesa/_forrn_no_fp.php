...
$this->registerJsFile(Url::to('@web/js/formDespesaCliente.js'),['position' => \yii\web\View::POS_END]);
...
<div class="row">
   ...
   <div class="row">
     ...
     <div class="col-md-3">
    <?= $form->field($model, 'numero')->textInput(['readonly' => $readonly,'maxlength' => true]) ?>
    ...
    <?= $form->field($model, 'dsp_processo_id')->widget(Select2::classname(), [
        ...
        'options' => ['placeholder' => 'Selecione ...','id'=>'dsp_processo_id','disabled'=>(!$model->isNewRecord&&$readonly)],
        'pluginOptions' => [
            'allowClear' => true,
        ],
...
<div class="titulo-principal"> <h5 class="titulo">DETALHES DA DESPESA</h5><div id="linha-longa"><div id="linha-curta"></div></div> </div>
<div class="row">
                 ...
                 <?php foreach ($modelsDespesaItem as $i => $item): ?>
                        ...
                        ?>
                          ...
                          <td style="width: 250px">
                           ...
                           <?= $form->field($item, "[{$i}]valor_iva")->textInput(['maxlength' => true])->label(false) ?></td>
                        <td style="width: 250px"> 
                            <?=$form->field($item, "[{$i}]cnt_plano_terceiro_id")->dropDownList(
                                ArrayHelper::map(PlanoTerceiro::find()->orderBy('nome')->all(), 'id', 'nome'),
                            ['prompt'=>' ']
                            )->label(false);?>
                            </td>
                        </tr>
    ...
    <table class="table table-hover">
       <tr>                
            <th style="width: 55%">Total<?=Html::hiddenInput('hidemCount', $i,['id'=>'hidemCount','class' => 'form-control']);?></th>
            <th></th>
            <th></th>
            <th><?= $form->field($model, 'valor')->textInput(['id'=>'valor','readonly' => true])->label(false) ?></th>
          </tr>
    </table>
