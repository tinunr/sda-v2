...
class ReportDeverSearch extends Processo
{
    public $documento_id;
     ...
     */
    public function adeverProcesso($params)
    { 
        /**
        ...
        $query = (new \yii\db\Query())
            ->select([
                ...
                'saldo'=>'(P.total_pago-P.valor_recebido_para_despesa)'
            ])
            ->from(['P' => $query10]) 
            ->groupBy('P.id')
            ->orderBy(['P.id'=>SORT_DESC])
			 ->where(['>=','valor_recebido_para_despesa',0])
            ->having(['>','saldo',0]);
...
// print_r(dataProvider);die();
     ...
     * @return ActiveDataProvider
     */
    public function adeverFP($params)
    {
